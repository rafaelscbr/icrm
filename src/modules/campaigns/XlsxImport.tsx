  ...
  if (imported) {
    return (
      <div className="flex flex-col items-center gap-3 py-8">
        ...
        <p className="text-sm font-medium text-t1">Importação concluída!</p>
      ...
      {!preview.length && (
        ...
        >
          ...
          <div className="text-center">
            <p className="text-sm font-medium text-t2">Arraste seu arquivo ou clique para selecionar</p>
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-t1">{filename}</span>
            </div>
            <button onClick={reset} className="text-t4 hover:text-t2 transition-colors cursor-pointer">
              <X size={15} />
            </button>
          </div>
            ...
            <div className="max-h-48 overflow-y-auto">
              {preview.slice(0, 50).map((l, i) => (
                <div key={i} className="grid grid-cols-3 gap-0 px-4 py-2.5 border-b border-line text-sm last:border-0">
                  <span className="text-t1 truncate">{l.name}</span>
                  <span className="text-t3 truncate tabular-nums">{formatPhone(l.phone)}</span>
                  <span className="text-t3 truncate">{l.email ?? '—'}</span>
                </div>
              ))}
              {preview.length > 50 && (
                <div className="px-4 py-2 text-xs text-t4 text-center">+ {preview.length - 50} mais leads</div>
              )}
            </div>
          </div>

-- ─── 066: o Pulse enxerga a prospecção por ligação ───────────────────────────
--
-- Ligação vira um evento de primeira classe no painel: aparece no feed, no
-- gráfico por hora e no radar de corretores, com contador próprio.
--
-- kind 'ligacao' separado de 'interacao' de propósito. Atendimento (interacoes)
-- mede conversa com lead do funil; ligação de prospecção é toque em base fria.
-- Somar os dois num número só faria um dia de 60 discagens parecer um dia de 60
-- atendimentos — e essa é justamente a confusão que o painel existe para evitar.
--
-- Redefine as duas RPCs por inteiro (CREATE OR REPLACE não aceita remendo).
-- Base: migração 057 para o snapshot, 059 para o resumo do dia.
...
with
  ...
  ev as (
    ...
    -- atividade de campanha de disparo (disparo, parecer, transferência…)
    select
    ...
    -- ligações da prospecção ativa. sub_tipo carrega o desfecho, então o feed
    -- distingue "ligou" de "ligou e o cliente demonstrou interesse".
    select
      ('call-' || cl.id), null::text, cl.called_at, 'ligacao'::text, cl.broker_id, cl.contact_name,
      null::text, null::text, cl.outcome, null::text, null::numeric, null::text
    from call_logs cl
    cross join b
    where cl.called_at >= b.day_start
      and cl.called_at <  b.day_end
  ...
  corretores as (
    select
      ...
      coalesce(pb.ligacoes, 0)   as ligacoes,
      pb.ultima
    from profiles p
...
-- ── Resumo do dia (2ª página do carrossel) ───────────────────────────────────

-- 054: tempos de resposta no iCRM Pulse
--
-- Duas métricas que o Pulse passa a exibir:
--   1. tempo entre o lead ENTRAR e receber o 1º contato
--   2. tempo entre o 1º e o 2º contato (a segunda tentativa / followup)
--
-- Ambas em MINUTOS ÚTEIS. Medir em tempo corrido seria enganoso: um lead do
-- Meta que cai às 3h da manhã e é atendido às 9h07 não demorou 6 horas —
-- demorou 7 minutos, porque o relógio do SLA só corre dentro do expediente.
-- É a mesma janela que sla_deadline() já usa para calcular o prazo.

-- ─── business_minutes: o inverso de sla_deadline ─────────────────────────────
-- Quantos minutos ÚTEIS se passaram entre dois instantes.
-- Janela: Seg–Sex 09:00–18:00, Sáb 09:00–13:00 (America/Sao_Paulo).
-- Domingo não conta. Intervalo invertido devolve 0.
create or replace function public.business_minutes(p_from timestamptz, p_to timestamptz)
returns numeric
language sql
immutable
...
as $$
  with lim as (
    select
      (p_from at time zone 'America/Sao_Paulo') as f,
      (p_to   at time zone 'America/Sao_Paulo') as t
  ),
  dias as (
    select d
    from lim, generate_series(date_trunc('day', lim.f), date_trunc('day', lim.t), interval '1 day') d
    where lim.t > lim.f
  ),
  janelas as (
    select
      d + interval '9 hours' as ini,
      d + (case when extract(dow from d) = 6 then interval '13 hours' else interval '18 hours' end) as fim
    from dias
    where extract(dow from d) <> 0
  )
  select coalesce(sum(
    greatest(0, extract(epoch from (
      least(j.fim, lim.t) - greatest(j.ini, lim.f)
    )) / 60.0)
  ), 0)::numeric
  from janelas j, lim
$$;

grant execute on function public.business_minutes(timestamptz, timestamptz) to authenticated;

-- Índice para a varredura de 1ª/2ª interação por lead
create index if not exists idx_li_lead_contato
  on public.lead_interactions (lead_id, interacted_at)
  where type in ('ligacao', 'whatsapp', 'email', 'visita', 'reuniao');

-- ─── pulse_snapshot ganha o bloco `tempos` ───────────────────────────────────

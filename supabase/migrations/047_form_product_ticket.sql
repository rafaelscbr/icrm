-- 047: Produto e ticket por formulário Meta
--
-- O admin define, por formulário, o produto que ele representa e o ticket médio.
-- Quando um lead entra por esse formulário, o process_meta_lead já preenche
-- property_name (produto) e average_ticket (ticket) — alimentando a previsão de VGL.

alter table public.meta_form_routing add column if not exists product_name   text;
alter table public.meta_form_routing add column if not exists product_ticket numeric;

-- process_meta_lead: passa a herdar produto + ticket da regra do formulário
...
BEGIN
  ...
  INSERT INTO leads (
    ...
    property_name, average_ticket, notes, sla_due_at,

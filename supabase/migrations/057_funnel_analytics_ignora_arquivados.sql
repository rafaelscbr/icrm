-- 057: lead_funnel_analytics passa a ignorar leads arquivados
--
-- `base_leads` filtrava apenas escopo e período — leads com discard_reason
-- entravam na coorte. Resultado: a coluna "Alcançaram" do funil (Dashboard e
-- aba Conversão) somava leads descartados, inflando volume e distorcendo a
-- conversão entre etapas.
--
-- Regra do produto: todo total de leads considera apenas leads ATIVOS.
-- Leads arquivados/descartados saem de qualquer contagem.
--
-- As demais RPCs (dashboard_overview, dashboard_extras) já filtravam.
-- Só o corpo de base_leads muda; a assinatura e o retorno são idênticos.
...
with
  ...
  -- Maior ordinal tocado por evento (from/to) por lead
  ev_max as (
    select li.lead_id, max(gs.ord) as max_ord
    from lead_interactions li
    join base_leads bl on bl.id = li.lead_id
    cross join lateral (values (li.from_stage), (li.to_stage)) as v(st)
    join stages gs on gs.stage = v.st
    where li.type = 'stage_change'
    group by li.lead_id
  ),
  -- Profundidade máxima = etapa atual vs maior etapa por evento
  lead_max as (
    select bl.id as lead_id, bl.origin, bl.broker_id,
      greatest(cs.ord, coalesce(em.max_ord, 0)) as max_ord
    from base_leads bl
    join stages cs on cs.stage = bl.funnel_stage
    left join ev_max em on em.lead_id = bl.id
  ),

  -- Tempo em etapa: linha do tempo (criação + cada entrada de etapa por evento)
  timeline as (
  ...
  -- Agregações por origem / corretor (alcançou etapa S = max_ord >= S.ord)
  origin_total as (select origin, count(*)::int as total from lead_max group by origin),
  origin_stage as (
    select lm.origin, s.stage, count(*)::int as cnt
    from lead_max lm join stages s on s.ord <= lm.max_ord
    group by lm.origin, s.stage
  ),
  broker_total as (select broker_id, count(*)::int as total from lead_max group by broker_id),
  broker_stage as (
    select lm.broker_id, s.stage, count(*)::int as cnt
    from lead_max lm join stages s on s.ord <= lm.max_ord
    group by lm.broker_id, s.stage
  )
...
select jsonb_build_object(
  ...
  'funnel', (
    select jsonb_agg(jsonb_build_object(
      'stage',   s.stage,
      'reached', (select count(*)::int from lead_max lm where lm.max_ord >= s.ord),
